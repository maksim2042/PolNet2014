...
__author__ = """Thomas Aynaud (thomas.aynaud@lip6.fr); Modified by Maksim Tsvetovat (maksim@tsvetovat.org)"""
#    Copyright (C) 2009 by
...
def plot_community(G,filename='communities.pdf'):
    ...
    for com in set(partition.values()) :
        ...
        net.draw_networkx_nodes(G, pos, list_nodes, node_size =70,
                                    node_color=colors[count], with_labels=True)
...
def plot_partitions(G,filename):    
    ...
    for i,sub in enumerate(subgraphs):
        ...
        count+=1
        if count >= len(colors): count=0
        plot.figure(num=None, figsize=(16, 12), dpi=180, facecolor='k', edgecolor='b')
        plot.axis('off')
        net.draw_networkx(sub,node_size=ns,node_color=colors[count],with_labels=True)
        plot.savefig(filename+str(i)+'.pdf')
        plot.clf()
        
        w=csv.writer(open(filename+'_list_'+str(i)+'.csv','wb'))
        w.writerows(sorted_dict(d))
        w.writerows(sub.edges(data=True))
